<template>
  <div class="chat">
    ...
    <div v-title :data-title="userName"></div>
      ...
      <div class="chat-record">
        <div class="chat-item" v-for="(item, index) in recordList" :key="index">
          ...
          <div :class="[item.isMe?'right':'left']">
            <img :src="item.isMe ? item.headImg : headImg " alt="" class="user-head">
...
export default {
  ...
  methods: {
    ...
    search(type) {
      chatApi.getMsgRecord(type == 'next' ? this.nextForm : this.prevForm).then(async res => {
        if (res.length > 0) {
          for (let i = 0; i < res.length; i++) {
            res[i].isListening = false
            if (i === res.length - 1) {
              res[i].showTime = true
              // res[i].time = `创建于${new Date(res[i].createTime).toLocaleString}`
            } else if ((res[i + 1].createTime - res[i].createTime) >= 5 * 1000 * 60) {
              res[i].showTime = true
            }
            if (res[i].fromId !== this.prevForm.openId) {
              res[i].isMe = true
              if (!this.clientImg) {
          ...
          switch (type) {
            case 'init': { // 首次进入 第一次拉取到信息后 滚到底
              this.recordList = this.recordList.concat(res.reverse())
              this.scrollToBottom()
              break
            }
            case 'prev':
              this.recordList = res.reverse().concat(this.recordList)
              this.isLoading = false
              break
            case 'next':
              this.recordList = this.recordList.concat(res.reverse())
    ...
    searchPrev(type) {
      chatApi.getMsgRecord2(this.prevForm).then(async res => {
        if (res.length > 0) {
          for (let i = 0; i < res.length; i++) {
            res[i].isListening = false
            if (i === res.length - 1) {
  ...
  created() {
    ...
    if (this.accessUserId == 0) {
      this.$dialog.confirm({
        ...
        let query = JSON.parse(JSON.stringify(this.$route.query))
        this.accessUserName = localStorage.getItem('name')
        query.accessUserId = 1
        query.accessUserName = localStorage.getItem('name')
        this.accessUserId = 1
        this.$router.replace({
          query
